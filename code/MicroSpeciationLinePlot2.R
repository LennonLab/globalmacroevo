...
## Modified to include Sepkoski origination intensities
## Ford Fishman
library(scales)
library(MASS)
library(grid)
...
EE <- c(rep(0,EeStart-1),rel_ext)

mod <- c(0,0.1,0.5,0.9)
# Simulate, but replace random background extinction levels 
# with "known" extinction rates for macroorganisms for "recent" years
for(t in 2:4000) {
  ep <- rnorm(n = epsilon, mean = epsilon, sd = 0.1) # give an epsilon near the actual epsilon for all simulations per time step
  ...
  if (t >= EeStart){ # when extinction data starts, use it in addition to background extinction
    ...
    if (t < 4000){
      # ep <- avg_ext[t - EeStart + 1]
      ep <- ep*(1-mod) + rel_ext[t - EeStart + 1]*mod
...
p2 <- ggplot(data = df, aes(y = logrichness, x = time, group = as.character(relative)), color = "black") + 
  ...
  scale_y_continuous("Taxon richness", breaks = seq(0,32,2), limits = c(10,14),labels = math_format(10^.x),expand = c(0,0)) +
...
p3 <- ggplot(data = df, aes(y = extinction, x = time, group = as.character(relative))) +
  ...
  scale_x_reverse("Millions of years ago",limits = c(800,0), expand = c(0,0)) +
  scale_y_log10("Relative Extinction\n(Macro-organisms)") +
  annotation_logticks(sides = "l") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
